<?php
  ...
  class leerExcel extends modelADG {
    function readxlsx() {
      ...
      if($area == 3) {
        $ruta = "C:/xampp/htdocs/Proyecto/Web/Compras/PAF/";
      }
      else if($area == 4) {
        $ruta = "C:/xampp/htdocs/Proyecto/Web/Ventas/PAF/";
      }
      else if($area == 5) {
        $ruta = "C:/xampp/htdocs/Proyecto/Web/Sistemas/PAF/";
      }
      ...
      if($nfsa == 0) {
        if ($rsa == 2 || $rsa == 4 | $rsa == 6) {
          ...
          $mes = $_POST['mes2'];
          $anho = $_POST['anho2'];
          $namef = $_FILES['carga_archivo2']['name'];
          $notas = $_POST['notas2'];
          $statusA = 1;
          $status = 1;
          ...
          // if ($guarda) {
          $carga_archivo = $ruta.basename($_FILES['carga_archivo2']['name']);
          if(file_exists($ruta.$namef)) {
            return $alerta = modelADG :: alertas('El archivo ya existe!',$url);
          }
          else {
            if (move_uploaded_file($_FILES['carga_archivo2']['tmp_name'], $carga_archivo)) {
